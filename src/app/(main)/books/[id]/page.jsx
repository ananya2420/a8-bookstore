...
const BookDetailsPage = ({ params }) => {
  ...
  const handleBorrow = () => {
    ...
    // 3. Success logic if user is logged in
    toast.success(`"${book.title}" successfully borrowed! 📚`);
  };

  // Handle case where book ID doesn't exist in JSON
  if (!book) {
    return (
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mt-6 space-y-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-gray-600">
            <span className="font-semibold text-gray-900">Category:</span> {book.category}
          </p>
          <p className="text-sm text-gray-500">
            <span className="font-semibold">Stock:</span> {book.available_quantity} copies left
          </p>
        </div>
